...
import os from 'os';//操作系统的主机名os.hostname()
...
const validateUser = (params: Params) => {
  ...
  if (!username) {
    const error: ResponseError = new Error("Le nom d'utilisateur est obligatoire");
    error.status = 400;
    throw error;
  }
  if (!password) {
...
const hashcode = (data: string) => {
  return AES.encrypt(JSON.stringify(data), os.hostname()).toString();
};
